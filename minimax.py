...
def minimax_block_action(currDepth: int, targetDepth: int, action: ActionType, p_id:int, cAction: CounterDecisions, counter_actor_id: int, \
    ...
    for counter in possible_counters: 
        if counter == CounterDecisions.DoNothing:
            ...
            _,successscore = minimax_action(currDepth, targetDepth, (p_id+1)%num_players, influence, bank, p_view)
            influence[counter_actor_id] += 1
            bank[p_id] -= 2

            # checking the score in case of a failure => when counteractor is a duke
            influence[p_id] -= 1
            _,failurescore = minimax_action(currDepth, targetDepth, (p_id+1)%num_players, influence, bank, p_view)
            influence[p_id] += 1
...
class MinimaxPlayer(HeuristicPlayer):
    ...
    def select_action(self) -> ActionType:
        ...
        for pi in self.player_view.players:
            ...
            bank.append(pi.bank)
        ((action,target),_) = minimax_action(0, self.targetDepth, self.player_view.players.index(self), influence, bank, self.player_view)
        self.target = target 
        return action
